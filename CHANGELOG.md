...
### Changed
- **A non-2xx *page* status is no longer treated as a successful enrichment.** Firecrawl v2 reports the origin's own HTTP status in `data.metadata.statusCode`, and a 2xx from the API does not imply the page fetch worked: verified against upstream 2.11.162, scraping a URL whose origin returns 404 yields HTTP 200, `success: true`, `statusCode: 404` and the *error page's* text as `markdown`. Without this check that error page was cached in Valkey for 6h as the job description, indistinguishable to the caller from a real one. v1 reports no `statusCode`, so the guard is a no-op there rather than a guess — a non-integer or absent value means "unknown", never "failed".
