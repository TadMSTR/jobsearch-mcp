...
- Stored profiles are trimmed before being sent to the scoring prompts — only `summary`, `skills`, `experience`, `certifications` and `target_roles` are included. `notification_email`, `work_authorization`, `salary_min`/`salary_max`, `education` and contact details were being sent on every call and never used by the rubric. Measured: 1,121 → 1,013 input tokens (−9.6%) on a representative profile.
